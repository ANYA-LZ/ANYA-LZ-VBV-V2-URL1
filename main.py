...
def process_payment(card, month, year, cvv, accessToken):
    try:
        ...
        token_result = get_token(card, month, year, cvv, random_person, accessToken)
        if 'tokencc_b' in token_result:
            data = json.loads(token_result)
            token = data['data']['tokenizeCreditCard']['token']
            result = check_card(token, random_person, card, accessToken)
            return result
        else:
...
def parse_response(card, month, year, cvv, accessToken):
    response = process_payment(card, month, year, cvv, accessToken)
...
def index():
    if request.method == 'POST':
        ...
        status, response = parse_response(card_number, exp_month, exp_year, cvv, accessToken)
